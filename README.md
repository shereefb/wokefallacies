...
├── *.html              # Individual fallacy pages (generated)
...
├── generate-pages.js   # Static site generator
...
Each fallacy has its own static HTML page for better SEO and instant loading. When you add or modify fallacies in `fallacies.json`, regenerate the pages:

```bash
node generate-pages.js
```

This creates individual HTML files (e.g., `disparity-discrimination.html`) for each fallacy with:
- Proper page titles and meta descriptions
- Open Graph tags for social sharing
- No "Redirecting..." flash when loading

## Clean URLs

The site uses clean URLs without hash fragments (e.g., `domain.com/disparity-discrimination` instead of `domain.com/#/disparity-discrimination`). This works through:

1. **Static HTML pages** for each fallacy (generated by `generate-pages.js`)
2. **Client-side routing** in `app.js` using the History API
3. **404.html redirect** as a fallback for GitHub Pages
